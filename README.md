...
### SDK changelog metadata

The SDK PR workflow writes `.changelog-pending/*.md` fragments from deterministic release entries generated by [`scripts/sdk-release-metadata.mjs`](scripts/sdk-release-metadata.mjs). Entries answer three questions: where the change occurred (`scope`), how severe it is (`feat!`, `feat`, or `fix`), and what changed. Known scopes also include `docs_url`, which the changelog renderer uses to link scope names to WorkOS API reference pages.

Severity is kept end-to-end as the paper trail — `entries_json`, the `oagen-diagnostics-<lang>` artifact (`classify-entries.json`, pre-strip `pr-body.md`), a notice and a section in the job's step summary — but nothing that lands in the SDK repo carries the conventional-commit breaking marker: [`scripts/open-batch-pr.mjs`](scripts/open-batch-pr.mjs) removes the `!` from commit subjects, the PR title and the PR body right before opening the PR, and the release-notes fragment header is written without it. A regeneration is therefore at most a minor release; a PR that genuinely needs a major bump is written by hand.

Each entry includes `scope_sources` and `scope_candidates` so the chosen scope can be audited. Model and enum scope resolution uses parsed IR references when no explicit symbol taxonomy matches; if the symbol taxonomy and IR references disagree, the script emits a warning with both values. Automated runs pass `--strict-scopes`, which fails the workflow if a user-facing entry resolves to the fallback `sdk` scope or lacks `docs_url` metadata.

In automation, `.github/workflows/generate-prs.yml` provides the structured `oagen diff`, parsed old/new IR, compat report, and changed SDK files. The script outputs JSON entries; the workflow creates the PR and adds the PR link/number when writing the changelog fragment.

For manual repair of existing fragments, pass the `openapi-spec` workflow commit that triggered the SDK PR. The script uses the spec state at that commit and diffs it against the previous spec-changing commit.

```bash
# JSON entries, useful for inspection
node scripts/sdk-release-metadata.mjs \
  --spec-commit dee95fc \
  --sdk-repo ../backend/workos-dotnet

# Full markdown fragment for replacing an existing .changelog-pending file
node scripts/sdk-release-metadata.mjs \
  --spec-commit dee95fc \
  --sdk-repo ../backend/workos-dotnet \
  --format changelog \
...
`--pr-number` and `--pr-url` only affect manual `--format changelog` output: they render the first fragment line with a PR link. Automated runs do not pass them because the workflow already gets the PR URL from `gh pr create`.

Run `node scripts/sdk-release-metadata.mjs --help` for all options.
...
`.github/workflows/resolve-sync-conflicts.yml` keeps autogenerated spec PRs
mergeable when their only conflict is `.last-synced-sha`. It compares the two
source commits in `workos/workos`, keeps the descendant SHA, and merges `main`
into the PR with a GitHub-signed bot commit. Other conflicts and divergent source
history still require manual review; SDK validation and approval requirements
are unchanged.

The workflow runs trusted code from `main`, never PR code. The existing SDK bot
App needs contents write and PR read access to this repository, plus contents
read access to `workos/workos`. Its branch updates trigger SDK validation again.

Preview locally with an authenticated `gh` that can read both repositories:

```bash
node scripts/resolve-sync-conflicts.mjs --pr 184
node --test scripts/__tests__/resolve-sync-conflicts.spec.mjs
```

The script is read-only on GitHub unless `--write` is supplied; writes require a
bot installation token so GitHub can sign the merge commit. Manual workflow runs
also default to dry-run. A new `pull_request_target` workflow must land on `main`
before it can resolve PRs automatically; local previews work before then.
